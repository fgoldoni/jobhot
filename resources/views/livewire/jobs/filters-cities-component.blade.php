<div>
    @foreach($rows as $row)
        <div class="hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 cursor-pointer">
            <input id="filter-cities-{{ $row->city_id }}"  wire:model="selected" value="{{ $row->city_id }}" type="checkbox" class="h-4 w-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500">
