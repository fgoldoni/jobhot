    ...
    @foreach($rows as $items)
        @foreach($items as $row)
            <div class="hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 cursor-pointer">
                <input id="filter-cities-{{ $row->country_id }}"  wire:model="selected" value="{{ $row->country_id }}" type="checkbox" class="h-4 w-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500">
                <label for="filter-cities-{{ $row->country_id }}" class="flex-1 ml-3 text-sm text-gray-600">{{ $row->country->name }}</label>
